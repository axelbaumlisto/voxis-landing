...
const architectureSteps = [
  {
    ...
    title: "Global Hotkey",
    subtitle: "LAYER 1 // SURFACE OS",
    desc: "Intercepts your designated hotkey globally with zero lag, bypassing app sandboxes to capture intent instantly.",
  ...
  },
  {
    ...
    title: "Rust Audio Core",
    subtitle: "LAYER 2 // UNDER THE HOOD",
    desc: "Captures raw audio from ALSA/PipeWire via CPAL. The stream is memory-safe, buffered asynchronously, and encoded to WAV.",
  ...
  },
  {
    ...
    subtitle: "LAYER 3 // CLOUD INFERENCE",
    desc: "Audio hits Groq LPU (whisper-large-v3) or OpenAI. The neural network parses technical jargon and returns text in milliseconds.",
  ...
  },
  {
    ...
    title: "Auto-Type Simulation",
    subtitle: "LAYER 4 // SURFACING BACK",
    desc: "The Rust orchestrator receives the payload and simulates native keystrokes, typing the result directly into your active window.",
...
export default function Home() {
  ...
  // Calculate stack spread
  const stackSpread = useTransform(scrollYProgress, [0, 0.1], [0, 100]);
    ...
    <main className="min-h-screen relative flex flex-col items-center bg-black overflow-hidden">
      ...
      <nav className="w-full max-w-6xl mx-auto px-6 py-6 flex justify-between items-center absolute top-0 left-1/2 -translate-x-1/2 z-50 backdrop-blur-sm bg-transparent border-b border-white/5">
        ...
        <div className="flex items-center gap-6 text-sm text-zinc-300 font-medium">
          <a href="#architecture" className="hover:text-white transition-colors hidden md:block">Architecture</a>
          <a href="https://docs.voxis.top" className="hover:text-white transition-colors hidden md:block">Documentation</a>
          ...
          <div className="flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full border border-white/10 ml-4 backdrop-blur-md">
            ...
            <a href="/ru" className="hover:text-white transition-colors">RU</a>
          </div>
        </div>
      </nav>
      ...
      <section className="w-full min-h-screen flex flex-col items-center justify-center text-center px-4 relative z-10 pt-20">
        ...
        >
          Speak your code. <br /> Write at lightspeed.
        </motion.h1>
          ...
          <div className="w-full md:w-1/2 h-[60vh] md:h-full flex items-center justify-center perspective-1500 z-10">
            ...
            >
              {architectureSteps.map((step, i) => {
                ...
                // Base isometric offset. When spread out, layer 0 is highest (negative Z), layer 3 is lowest.
                // Actually, in CSS 3D, translateZ moves it towards the viewer.
                // So Layer 0 (Top) should have the highest translateZ.
                const baseZ = (3 - i) * 120; 
